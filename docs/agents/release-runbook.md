# Formal Release Runbook

## Purpose
M2で正式リリース (`vX.Y.Z`) を公開し、タグ・Release・CHANGELOG・受け入れ状態を一致させる。
加えて Maven Central への配布を行う。

## Target (current)
- Version: `v0.1.0`
- Repository: `shgnaka/geo-event-trigger-core-sdk`

## Preconditions
- Current branch is `main`
- Working tree is clean
- GitHub CLI authenticated
- GitHub Actions `release` environment has required reviewers configured
- GitHub repository secrets are configured:
  - `CENTRAL_PORTAL_USERNAME`
  - `CENTRAL_PORTAL_PASSWORD`
  - `SIGNING_KEY` (ASCII-armored private key)
  - `SIGNING_PASSWORD`
- Required checks all green:
  - `lint`
  - `unit`
  - `consistency`
  - `contract`
  - `compatibility`
  - `policy`
  - `security`

## Command
```bash
scripts/release.sh v0.1.0 shgnaka/geo-event-trigger-core-sdk
```

## Publish Pipeline Trigger
- Push semver tag (`vX.Y.Z`) to origin.
- Workflow: `.github/workflows/publish-central.yml`
- Gate policy:
  - quality gates green
  - `security-loop` high/critical = fail
  - release environment manual approval required

## Expected Output
- Annotated tag `v0.1.0` exists on origin
- GitHub Release for `v0.1.0` is published (`isDraft=false`, `isPrerelease=false`)
- Release URL is recorded in acceptance board
- Maven Central publish workflow completed successfully
- Security loop publish artifact exists (`security-loop-reports-publish`)

## Post Steps
1. Update `docs/agents/acceptance-board.md` M2 checklist
2. Ensure `CHANGELOG.md` has finalized `v0.1.0` notes
3. Validate release note format with `docs/agents/release-notes-style-guide.md`
4. Confirm open issues/PRs relevant to release are triaged
